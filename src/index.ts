...
export default new Bundler({
    ...
    bundle({options, logger, bundleGraph}) {
        ...
        const packageJSON = resolveConfigSync(options.inputFS, "", ["package.json"])
        ...
        bundleGraph.traverse({
            enter: (node, context: any) => {
                ...
                if (
                    ...
                    resolution?.isInline ||
                    splitPaths.includes(resolution?.filePath)
                ) {
    ...
    optimize({bundleGraph}) {
        ...
        let candidateBundles: Map<string,
            {
                assets: Array<Asset>,
